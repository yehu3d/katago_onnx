...
vector<NNEvaluator*> Setup::initializeNNEvaluators(
  ...
  for(size_t i = 0; i<nnModelFiles.size(); i++) {
    ...
    string onnxfile;
    if(cfg.contains("onnxFile" + idxStr))
      onnxfile = cfg.getString("onnxFile" + idxStr);
    else if(cfg.contains("onnxFile"))
      onnxfile = cfg.getString("onnxFile");
    else
      std::cout << "Error ! No onnx model file was selected!" << endl;
    logger.write("using onnxFile" + idxStr + " = " + onnxfile);
